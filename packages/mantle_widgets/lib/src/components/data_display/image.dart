// Contracts are single-method by design so themes only implement `root`.
// ignore_for_file: one_member_abstracts

import 'package:flutter/widgets.dart';
import 'package:mantle_annotations/mantle_annotations.dart';
import 'package:mantle_core/mantle_core.dart';
import 'package:mantle_widgets/src/components/data_display/types.dart';

part 'image.g.dart';

/// Source-of-truth contract for MImage: network/asset image with radius and fallback.
@MantleComponent(
  name: 'MImage',
  variants: [MantleVariant('default')],
  properties: [
    MantleProperty<String>('src', nullable: true),
    MantleProperty<MantleImageProvider>('image', nullable: true),
    MantleProperty<Widget>('fallback', nullable: true),
    MantleProperty<BoxFit>('fit', nullable: true, defaultValue: BoxFit.cover),
    MantleProperty<BorderRadiusGeometry>(
      'radius',
      defaultBuilder: _zeroRadius,
    ),
    MantleProperty<double>('width', nullable: true),
    MantleProperty<double>('height', nullable: true),
  ],
)
abstract class MantleImageContract {
  /// Builds the image or fallback.
  @MantleComponent.slot()
  Widget root(MImageContext context);
}

BorderRadiusGeometry _zeroRadius(BuildContext context) => BorderRadius.zero;
